...
**정식 탐색 (1h/2h/4h × 144개 설정, 본페로니 + 검증기간, 메이커 0.09%)**

    본페로니 임계값 |t| >= 3.70
    보정 없이 |t|>=2만 봤다면 통과: 0개   (우연 기대치 3.6개)
    본페로니 통과 0개 · 검증기간까지 통과 0개

우연이라면 3.6개는 나왔어야 하는데 **하나도 안 나왔다.** 상위권이 전부
음수이고 탐색기간 t가 -3.21까지 내려간다. 우위가 없는 정도가 아니라
비용을 감안하면 **믿을 만하게 지는 쪽**이다.
...
class BoxRangeStrategy(Strategy):
    ...
    retired = (
        ...
        "1h/2h/4h 144개 설정을 본페로니+검증기간으로 훑어 0개 통과 — "
        "보정 없이 |t|>=2만 봐도 0개다(우연 기대치 3.6개). 상위권이 전부 음수"
    )
